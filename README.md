# 📦 Supply Chain Demand Forecasting & Inventory Optimization

<p align="center">
  <img src="https://img.shields.io/badge/Python-3.10+-3776AB?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white"/>
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>
  <img src="https://img.shields.io/badge/Status-Complete-1E8449?style=for-the-badge"/>
</p>

> **Personal Project · February 2026 · Mousumi Paul**

An end-to-end supply chain analytics solution covering **demand forecasting** and **inventory optimization** across 5 product categories using 12 months of historical sales data — achieving **88% forecast accuracy** and an estimated **22% reduction in excess inventory holding costs**.

---

## 📌 Table of Contents

- [Project Overview](#-project-overview)
- [Key Results](#-key-results)
- [Project Structure](#-project-structure)
- [Excel Workbook](#-excel-workbook)
- [Forecasting Models](#-forecasting-models)
- [Inventory Optimization](#-inventory-optimization)
- [Power BI Dashboard](#-power-bi-dashboard)
- [Setup & Usage](#-setup--usage)
- [Tech Stack](#-tech-stack)

---

## 🔍 Project Overview

This project simulates a real-world supply chain analytics workflow for a retail business managing 5 product categories:

| Code | Category |
|------|----------|
| P01 | Electronics |
| P02 | Apparel |
| P03 | Home & Kitchen |
| P04 | Sports & Outdoors |
| P05 | Beauty & Health |

**Two core problems solved:**

1. **Demand Forecasting** — Predict monthly unit demand using Moving Averages (3M, 6M) and Exponential Smoothing, then evaluate each model's accuracy using MAPE, MAE, and RMSE.

2. **Inventory Optimization** — Calculate optimal order quantities (EOQ), safety stock levels, and reorder points to minimize total inventory cost while maintaining a 95% service level.

---

## 🏆 Key Results

| Metric | Result |
|--------|--------|
| 📈 Overall Forecast Accuracy | **~88%** (best model per category) |
| 💰 Excess Holding Cost Reduction | **~22%** vs fixed-order-quantity baseline |
| 📦 Products with Optimized EOQ | **5 / 5** |
| ⚠️ Automated Low-Stock Alerts | **Live (Power BI + Excel)** |
| 🔢 Excel Live Formulas | **365 (0 errors)** |

### Forecast Accuracy by Category

| Category | Best Model | Accuracy |
|----------|-----------|----------|
| Electronics | Exponential Smoothing | ~90% |
| Apparel | Exponential Smoothing | ~88% |
| Home & Kitchen | 3-Month MA | ~89% |
| Sports & Outdoors | 6-Month MA | ~88% |
| Beauty & Health | Exponential Smoothing | ~91% |

### Inventory Parameters

| Category | EOQ (units) | Safety Stock | Reorder Point |
|----------|-------------|--------------|---------------|
| Electronics | 183 | 90 | 428 |
| Apparel | 242 | 74 | 355 |
| Home & Kitchen | 200 | 56 | 291 |
| Sports & Outdoors | 204 | 103 | 450 |
| Beauty & Health | 330 | 49 | 326 |

---

## 📁 Project Structure

```
supply-chain-forecasting/
│
├── 📊 excel/
│   └── SupplyChain_Forecast_Inventory.xlsx   ← 6-sheet model · 365 live formulas
│
├── 📈 powerbi/
│   ├── dax_measures.md                        ← 20+ DAX measures documented
│   └── SupplyChain_Dashboard_PLACEHOLDER.txt  ← Power BI rebuild guide
│
├── 📂 data/
│   ├── raw/
│   │   ├── sales_data_2024.csv                ← 60-row monthly sales dataset
│   │   └── inventory_params.csv               ← EOQ input parameters per product
│   └── processed/
│       ├── forecast_output.csv                ← Generated by notebook 02
│       └── inventory_optimization_output.csv  ← Generated by notebook 03
│
├── 📓 notebooks/
│   ├── 01_data_prep_eda.ipynb                 ← EDA, seasonality, quality checks
│   ├── 02_demand_forecasting.ipynb            ← MA + ES models, accuracy reports
│   ├── 03_inventory_optimization.ipynb        ← EOQ/ROP/SS + cost savings analysis
│   └── 04_full_pipeline_report.ipynb          ← End-to-end pipeline + KPI summary
│
├── 🐍 src/
│   ├── demand_forecasting.py                  ← Forecasting engine (MA + ES + metrics)
│   ├── inventory_optimization.py              ← EOQ/ROP/Safety Stock + cost curves
│   └── utils.py                               ← Helpers: formatting, export, reporting
│
├── 📤 outputs/
│   ├── charts/                                ← Auto-saved PNG visualizations
│   └── reports/                               ← Combined CSV reports
│
├── 📄 docs/
│   └── methodology.md                         ← Full formula derivations & assumptions
│
├── requirements.txt
├── .gitignore
└── README.md
```

---

## 📊 Excel Workbook

The workbook `SupplyChain_Forecast_Inventory.xlsx` contains **6 sheets** with **365 live formulas** and zero errors.

| Sheet | Description |
|-------|-------------|
| `Sales_Data` | Raw 12-month unit sales · MoM growth % · trend line chart |
| `Demand_Forecast` | 3MA, 6MA, Exp Smoothing per product + MAPE accuracy |
| `Inventory_Optimization` | EOQ, Safety Stock, ROP, annual holding & ordering costs |
| `Stock_Health` | Live alert dashboard: 🔴🟠🟡🟢 status + recommended actions |
| `Accuracy_Summary` | Best model by category · 88% target check |
| `Guide` | Color legend, formula reference, model navigation |

### Color Coding Convention

| Color | Meaning |
|-------|---------|
| 🔵 **Blue text** | Hardcoded inputs — safe to edit |
| ⚫ **Black text** | Formula outputs — do not modify |
| 🟢 **Green text** | Cross-sheet links |
| 🟡 **Yellow background** | Key assumption cells |
| **Teal highlight** | EOQ output values |
| **Green highlight** | Safety stock values |
| **Blue highlight** | Reorder point values |
| **Amber highlight** | Forecast months + excess cost flags |

---

## 📈 Forecasting Models

Three models are implemented and compared for each product category.

### 1. Moving Average (3-Month & 6-Month)

```
MA_t(n) = (A_t + A_{t-1} + ... + A_{t-n+1}) / n
```

- **3-Month MA** — Reacts quickly to recent demand shifts; good for volatile categories
- **6-Month MA** — Smooths seasonal noise; better for stable demand patterns

### 2. Exponential Smoothing (α = 0.3)

```
F_t = α × A_{t-1} + (1 - α) × F_{t-1}
```

Where α = smoothing parameter (0.3 default, tuned via sensitivity analysis). Higher α = more weight to recent data.

### 3. Accuracy Metrics

| Metric | Formula | Used For |
|--------|---------|----------|
| **MAPE** | Mean(\|A−F\|/A) × 100 | Model selection (primary) |
| **MAE** | Mean(\|A−F\|) | Absolute unit error |
| **RMSE** | √Mean((A−F)²) | Penalizing large errors |

---

## 📦 Inventory Optimization

### Economic Order Quantity (EOQ)

```
EOQ = √(2 × D × S / H)
```

| Variable | Description | Value |
|----------|-------------|-------|
| D | Annual demand (units) | Per product |
| S | Ordering cost per order | ₹2,500 |
| H | Holding cost per unit/year | Unit Cost × Holding % |

### Safety Stock

```
Safety Stock = Z × σ_demand × √(Lead Time / 30)
```

- Z = 1.65 (95% service level)
- Lead Time = 14 days

### Reorder Point

```
ROP = (Daily Demand × Lead Time in Days) + Safety Stock
```

### Annual Cost Model

```
Annual Holding Cost  = (EOQ/2 + Safety Stock) × H
Annual Ordering Cost = (D / EOQ) × S
Total Inventory Cost = Holding Cost + Ordering Cost
```

---

## ⚡ Power BI Dashboard

The dashboard has **5 pages** with **20+ DAX measures**. Full measure definitions are in [`powerbi/dax_measures.md`](powerbi/dax_measures.md).

**Dashboard Pages:**
1. **Executive Summary** — KPI cards, overall demand trend, alert banner
2. **Demand Forecast** — Actual vs forecast lines, MAPE accuracy matrix, model slicer
3. **Inventory Health** — Stock vs ROP vs Safety Stock, status table with conditional formatting
4. **Cost Analysis** — Holding vs ordering cost breakdown, 22% savings waterfall
5. **Low-Stock Alerts** — Dynamic color alerts, reorder recommendations, days-of-supply gauge

**Key DAX Measures include:**
- `[Rolling 3M Avg Demand]` — DATESINPERIOD lookback
- `[Products Below ROP]` — Dynamic COUNTX filter
- `[Stock Alert Color]` — Conditional hex color for formatting
- `[Low Stock Alert Message]` — Plain-language alert string
- `[Forecast Accuracy %]` — Linked to DimForecast table

---

## ⚙️ Setup & Usage

### Prerequisites
```bash
pip install -r requirements.txt
```

### Run Notebooks (Recommended)
```bash
# Step 1 – EDA & data validation
jupyter notebook notebooks/01_data_prep_eda.ipynb

# Step 2 – Demand forecasting + accuracy report
jupyter notebook notebooks/02_demand_forecasting.ipynb

# Step 3 – Inventory optimization + cost savings
jupyter notebook notebooks/03_inventory_optimization.ipynb

# Step 4 – Full pipeline + executive KPI summary
jupyter notebook notebooks/04_full_pipeline_report.ipynb
```

### Run Python Scripts Directly
```bash
cd src
python demand_forecasting.py       # → data/processed/forecast_output.csv + charts
python inventory_optimization.py   # → data/processed/inventory_optimization_output.csv + charts
```

### Use the Excel Model
1. Open `excel/SupplyChain_Forecast_Inventory.xlsx`
2. Go to `Inventory_Optimization` — update **blue cells** (current stock, lead time, costs)
3. Go to `Stock_Health` — live alert status updates automatically
4. All 365 formulas recalculate instantly on data change

### Rebuild Power BI Dashboard
1. Import CSVs from `data/processed/` as data sources
2. Create `DimDate` in Power Query
3. Set up relationships (see `powerbi/dax_measures.md`)
4. Paste all DAX measures from `powerbi/dax_measures.md`
5. Configure conditional formatting with `[Stock Alert Color]`

---

## 🛠 Tech Stack

| Tool | Purpose |
|------|---------|
| **Python 3.10+** | Forecasting engine, optimization logic, charting |
| **pandas / numpy** | Data manipulation and numerical computation |
| **matplotlib** | Forecast charts, EOQ cost curves, stock health visuals |
| **openpyxl** | Excel workbook creation with live formulas |
| **Microsoft Excel** | Interactive model with dynamic formulas & charts |
| **Power BI** | Live dashboard with DAX measures and low-stock alerts |
| **Jupyter Notebooks** | Step-by-step analysis and reporting |

---

## 📄 Documentation

Full methodology, formula derivations, and model assumptions are documented in [`docs/methodology.md`](docs/methodology.md).

---

## 📬 Contact

**Mousumi Paul**  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/mousumi-paul)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/mousumi-paul)
